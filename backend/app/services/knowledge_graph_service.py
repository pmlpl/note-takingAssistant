...
from app.models.kg import KGConceptDB, KGEdge, KGNode, KGRelationDB, KGStatusDB
...
async def build_knowledge_graph(
    db: AsyncSession, user_id: int, db_user: UserDB
) -> tuple[list[KGNode], list[KGEdge], dict]:
    result = await db.execute(
        select(NoteDB)
        .where(NoteDB.user_id == user_id, NoteDB.is_favorite)
    ...
    frequent_concepts = {
        name: data for name, data in all_concepts.items() if len(data["note_ids"]) >= MIN_CONCEPT_NOTE_FREQ
    }
...
async def get_kg_from_db(db: AsyncSession, user_id: int) -> Optional[tuple[list[KGNode], list[KGEdge], dict]]:
    result = await db.execute(select(KGStatusDB).where(KGStatusDB.user_id == user_id))
    status = result.scalar_one_or_none()
    if not status or status.status != "ready":
        return None

    result_concepts = await db.execute(select(KGConceptDB).where(KGConceptDB.user_id == user_id))
    concept_db_list = result_concepts.scalars().all()

    result_relations = await db.execute(select(KGRelationDB).where(KGRelationDB.user_id == user_id))
    rel_db_list = result_relations.scalars().all()

    result_notes = await db.execute(
        select(NoteDB).where(NoteDB.user_id == user_id, NoteDB.is_favorite).order_by(NoteDB.updated_at.desc())
    )
