...
async function handleGithubLogin() {
  ...
  try {
    ...
    if (res.authorize_url) {
      ...
      if (isDesktop) {
        // 桌面端：弹出独立窗口做 OAuth，避免主窗口被导航走
        const result = await window.electronAPI.oauth.startGithub(res.authorize_url)
        if (result.success && result.url) {
          const urlObj = new URL(result.url)
          // 后端最终会重定向到 FRONTEND_URL/login?token=xxx
          // 我们直接从最终跳转的 URL 中提取 token
          ...
          ElMessage.info(result.error || 'GitHub 登录已取消')
        }
      } else {
