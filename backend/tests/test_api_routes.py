...
def auth_token(client):
    ...
    client.post("/api/v1/user/register", json={"email": email, "password": password, "nickname": "testuser"})
...
def test_register_success(client):
    ...
    response = client.post("/api/v1/user/register", json={"email": email, "password": password, "nickname": "reguser"})
    assert response.status_code == 200
    data = response.json()
    assert "id" in data
...
def test_register_duplicate_email(client):
    ...
    client.post("/api/v1/user/register", json={"email": email, "password": password, "nickname": "dupuser"})

    response = client.post("/api/v1/user/register", json={"email": email, "password": password, "nickname": "dupuser2"})
...
def test_register_invalid_password(client):
    response = client.post(
        "/api/v1/user/register", json={"email": "invalid@example.com", "password": "short", "nickname": "invaliduser"}
    )
    assert response.status_code == 400
...
def test_login_wrong_password(client):
    ...
    client.post("/api/v1/user/register", json={"email": email, "password": password, "nickname": "wrongpwduser"})
...
def test_get_note_list_with_auth(client, auth_token):
    response = client.get("/api/v1/note/", headers={"Authorization": f"Bearer {auth_token}"})
    assert response.status_code == 200
    data = response.json()
...
def test_get_note_detail_with_auth(client, auth_token):
    create_response = client.post(
        "/api/v1/note/",
        headers={"Authorization": f"Bearer {auth_token}"},
        json={"title": "Detail Test Note", "content": "<p>Detail content</p>", "tags": "detail", "is_favorite": False},
    )
    note_id = create_response.json()["id"]

    response = client.get(f"/api/v1/note/{note_id}", headers={"Authorization": f"Bearer {auth_token}"})
    assert response.status_code == 200
    data = response.json()
...
def test_delete_note_with_auth(client, auth_token):
    create_response = client.post(
        "/api/v1/note/",
        headers={"Authorization": f"Bearer {auth_token}"},
        json={"title": "Delete Test Note", "content": "<p>Will be deleted</p>", "tags": "delete", "is_favorite": False},
    )
    note_id = create_response.json()["id"]

    delete_response = client.delete(f"/api/v1/note/{note_id}", headers={"Authorization": f"Bearer {auth_token}"})
    assert delete_response.status_code == 200

    get_response = client.get(f"/api/v1/note/{note_id}", headers={"Authorization": f"Bearer {auth_token}"})
...
def test_note_crud_without_auth(client):
    response = client.post(
        "/api/v1/note/",
        json={"title": "Unauthorized Note", "content": "<p>Should fail</p>", "tags": "unauth", "is_favorite": False},
    )
    assert response.status_code == 401
    ...
    response = client.put("/api/v1/note/1", json={"title": "Update", "content": "<p>Update</p>"})
    assert response.status_code == 401
